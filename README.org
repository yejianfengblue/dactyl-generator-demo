...
** Internal connector TRRS Micro USB
   ...
   The position of the L hook is not calculated correctly in some case. Check it carefully.
   For example with the default dactyl manuform parameters, the L hook is separated from keyboard case.
   It's a bit complicated to tell how to fix it. Consider external holder.
